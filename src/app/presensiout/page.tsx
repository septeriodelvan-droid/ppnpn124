...
export default function PresensiOutForm() {
    ...
    const handleCheckOut = async () => {
        ...
        if (!currentShift) {
            fail('Absen pulang belum berhasil. Shift aktif tidak ditemukan.');
            return;
        }

        if (!attendanceId) {
            fail('Absen pulang belum berhasil. Tidak ada sesi absen yang aktif.');
            return;
        }

        if (!attendanceDate) {
            fail('Absen pulang belum berhasil. Tanggal presensi tidak ditemukan.');
            return;
        }

        if (!location) {
            fail('Absen pulang belum berhasil. Lokasi belum terdeteksi.');
            return;
        }
        ...
        if (
            ...
            fail('Absen pulang belum berhasil. Anda harus mengisi dan Submit Logbook terlebih dahulu.');
            return;
        }
        ...
        if (!isValidLocation) {
            fail(
                `Absen pulang belum berhasil. Anda harus berada maksimal ${OFFICE_LOCATION.radius_m} meter dari KPPN Tebing Tinggi.`
            );
            return;
        }

        if (!photo) {
            fail('Absen pulang belum berhasil. Silakan ambil foto selfie terlebih dahulu.');
            return;
        }

        const now = new Date();

        // Jam standar hanya dipakai sebagai dasar perhitungan potongan.
        // Checkout tetap dapat dilakukan kapan saja.
        const scheduledEnd =
            currentShift === 'pagi'
        ...
        setSubmitNotification({
            type: 'info',
            message: 'Sedang memproses presensi pulang...',
        });
        ...
        try {
            ...
            const successMessage =
                earlyLeaveMinutes > 0
                    ? `Absen pulang BERHASIL. Anda pulang ${earlyLeaveMinutes} menit lebih awal. Total potongan waktu ${deductionMinutes} menit.`
            ...
            const message =
                err?.message ||
                'Absen pulang belum berhasil. Gagal menyimpan data presensi.';
